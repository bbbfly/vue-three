...
- ✅ **GRP-001: GroupContext 类型定义与 InjectionKey**
  - 在 `core/context.ts` 中定义 `GroupContext` 接口
  - 创建 `GroupContextKey` Symbol 注入键
  - 支持任意层级嵌套的上下文传递

- ✅ **GRP-002: 实现 useGroup composable**
  - 创建 `useGroup` 组合式函数
  - 自动注入 ThreeContext 和 GroupContext（支持嵌套）
  - computed 计算父对象：优先取父 Group，无则回退到 scene
  - provide GroupContext 供子组件消费
  - 完整的资源清理机制

- ✅ **GRP-003: TGroup 组容器组件实现**
  - 创建 `TGroup.vue` 组件
  - 支持 Object3D 基础属性：position / rotation / scale / visible
  - 支持 receiveShadow / castShadow / userData / name
  - **修复属性动态更新问题**：添加 onMounted 初始配置 + watch 响应式更新

- ✅ **GRP-004: useMesh 适配 GroupContext**
  - 注入 GroupContext，计算父对象
  - 模型添加/移除逻辑使用 parent.value

- ✅ **GRP-005: useLight 适配 GroupContext**
  - 注入 GroupContext，计算父对象
  - 光源添加/移除逻辑使用 parent.value

- ✅ **GRP-006: useLine 适配 GroupContext**
  - useLine / useLineLoop / useLineDashed / useLineSegments 均适配
  - 线条添加到父对象而非直接到 scene

- ✅ **GRP-007: useSprite 适配 GroupContext**
  - 注入 GroupContext，计算父对象
  - 精灵添加到父对象

- ✅ **GRP-008: useHelper 适配 GroupContext**
  - 注入 GroupContext，计算父对象
  - 辅助对象添加到父对象

- ✅ **GRP-009: useGLTFLoader 适配 GroupContext**
  - 注入 GroupContext，计算父对象
  - 加载的模型添加到父对象

- ✅ **GRP-010: useOBJLoader 适配 GroupContext**
  - 注入 GroupContext，计算父对象 parent = groupCtx?.group.value || scene.value
  - 模型添加/移除逻辑使用 parent.value

- ✅ **GRP-011: useFBXLoader 适配 GroupContext**
  - 注入 GroupContext，计算父对象 parent = groupCtx?.group.value || scene.value
  - 模型添加/移除逻辑使用 parent.value

- ✅ **GRP-012 & GRP-013: useCSS2DRenderer / useCSS3DRenderer 不需要适配**
  - CSS2D/CSS3D 渲染器管理的对象应添加到 scene 而非 Group
  - 这是正确的设计，无需修改

- ✅ **GRP-014: TGroup 组件添加到入口导出**
  - 导出 useGroup composable
  - 导出 TGroup 组件
  - 自动注册到 Vue 插件

**新增文件**:

- [packages/vue-three/src/core/context.ts](file:///d:/www/AI/%E9%A1%B9%E7%9B%AE/VueThreeV7/packages/vue-three/src/core/context.ts) - GroupContext 类型定义
- [packages/vue-three/src/composables/useGroup.ts](file:///d:/www/AI/%E9%A1%B9%E7%9B%AE/VueThreeV7/packages/vue-three/src/composables/useGroup.ts) - useGroup composable
- [packages/vue-three/src/components/TGroup.vue](file:///d:/www/AI/%E9%A1%B9%E7%9B%AE/VueThreeV7/packages/vue-three/src/components/TGroup.vue) - TGroup 组件

**修改文件**:

- [packages/vue-three/src/composables/useMesh.ts](file:///d:/www/AI/%E9%A1%B9%E7%9B%AE/VueThreeV7/packages/vue-three/src/composables/useMesh.ts) - 适配 GroupContext
- [packages/vue-three/src/composables/useLight.ts](file:///d:/www/AI/%E9%A1%B9%E7%9B%AE/VueThreeV7/packages/vue-three/src/composables/useLight.ts) - 适配 GroupContext
- [packages/vue-three/src/composables/useLine.ts](file:///d:/www/AI/%E9%A1%B9%E7%9B%AE/VueThreeV7/packages/vue-three/src/composables/useLine.ts) - 适配 GroupContext
- [packages/vue-three/src/composables/useSprite.ts](file:///d:/www/AI/%E9%A1%B9%E7%9B%AE/VueThreeV7/packages/vue-three/src/composables/useSprite.ts) - 适配 GroupContext
- [packages/vue-three/src/composables/useHelper.ts](file:///d:/www/AI/%E9%A1%B9%E7%9B%AE/VueThreeV7/packages/vue-three/src/composables/useHelper.ts) - 适配 GroupContext
- [packages/vue-three/src/composables/useGLTFLoader.ts](file:///d:/www/AI/%E9%A1%B9%E7%9B%AE/VueThreeV7/packages/vue-three/src/composables/useGLTFLoader.ts) - 适配 GroupContext
- [packages/vue-three/src/composables/useOBJLoader.ts](file:///d:/www/AI/%E9%A1%B9%E7%9B%AE/VueThreeV7/packages/vue-three/src/composables/useOBJLoader.ts) - 适配 GroupContext
- [packages/vue-three/src/composables/useFBXLoader.ts](file:///d:/www/AI/%E9%A1%B9%E7%9B%AE/VueThreeV7/packages/vue-three/src/composables/useFBXLoader.ts) - 适配 GroupContext
- [packages/vue-three/src/index.ts](file:///d:/www/AI/%E9%A1%B9%E7%9B%AE/VueThreeV7/packages/vue-three/src/index.ts) - 添加导出
...
<TCanvas>
  <TScene>
    <!-- 外层组 -->
    <TGroup :position="[0, 0, 0]" :scale="[1, 1, 1]">
      <TMesh>
        <TBox :args="[1, 1, 1]" />
      ...
      <TGroup :position="[2, 0, 0]">
        <TMesh>
          <TSphere :args="[0.5]" />
          <TMeshStandardMaterial :color="0x00ff00" />
        </TMesh>
        
        <!-- 三层嵌套 -->
        <TGroup :position="[1, 0, 0]">
          <TMesh>
            <TCone :args="[0.3, 0.8]" />
            <TMeshStandardMaterial :color="0x0000ff" />
          </TMesh>
        </TGroup>
      </TGroup>
    </TGroup>
    
    <!-- 未使用 TGroup，直接添加到 scene（向后兼容） -->
    <TMesh :position="[-2, 0, 0]">
      <TCylinder :args="[0.3, 1, 0.3]" />
      <TMeshStandardMaterial :color="0xffff00" />
    </TMesh>
  </TScene>
</TCanvas>
```
...
- ✅ **层级嵌套**：支持 scene → group → group → mesh 的任意层级结构
- ✅ **自动父对象识别**：子组件自动查找最近的 GroupContext，无则回退到 scene
- ✅ **向后兼容**：未使用 TGroup 的子组件仍直接添加到 scene
- ✅ **响应式属性更新**：position、rotation、scale、visible 等属性动态更新
- ✅ **完整资源清理**：组件卸载时自动从父对象移除并清理资源
...
## DEMO-024: TGroup 层级分组演示页面开发完成

**完成时间**: 2026-05-07
**完成内容**:

- ✅ **DEMO-024: TGroup 层级分组演示页面**
  - 创建 `GroupDemoView.vue` 完整演示页面
  - 三层嵌套 TGroup 结构：外层组(红色) → 中层组(绿色) → 内层组(蓝色)
  - 每层组包含不同颜色的 3D 对象
  - 独立圆柱体展示向后兼容性（直接添加到 scene）
  - 响应式动画效果：外层组脉冲、中层组旋转、内层组呼吸

**新增文件**:

- [packages/playground/src/views/GroupDemoView.vue](file:///d:/www/AI/%E9%A1%B9%E7%9B%AE/VueThreeV7/packages/playground/src/views/GroupDemoView.vue) - TGroup 演示页面

**修改文件**:

- [packages/playground/src/router/index.ts](file:///d:/www/AI/%E9%A1%B9%E7%9B%AE/VueThreeV7/packages/playground/src/router/index.ts) - 添加 /demo/group 路由
- [packages/playground/src/components/ComponentTree.vue](file:///d:/www/AI/%E9%A1%B9%E7%9B%AE/VueThreeV7/packages/playground/src/components/ComponentTree.vue) - 添加 TGroup 导航菜单

**访问方式**:

```
http://localhost:5173/#/demo/group
```
...
## SPR-019 ~ SPR-028: Sprite 组件开发完成

**完成时间**: 2026-04-29  
**完成内容**:

- ✅ **SPR-019: TSprite 精灵组件基础实现**
  - 遵循 TMesh 统一组件架构模式
  - 完整的 Vue 3 Composition API + <script setup>
  - 插槽支持子组件插入
  - 完整的 JSDoc 组件文档注释

- ✅ **SPR-020: TSprite 支持 config prop 配置驱动**
  - computed 自动检测 config prop
  - `config?: SpriteConfig` 完整配置对象
  - 独立属性与 config 对象二选一模式
  - config 变更自动响应式更新

- ✅ **SPR-021: TSprite 暴露 sprite 实例**
  - defineExpose 导出 sprite 实例 ref
  - 导出 setMaterial / setCenter / setMaterialConfig 方法
  - 支持父组件通过 ref 直接调用 API

- ✅ **SPR-022: TSprite 支持交互事件（click/pointer-\*）**
  - 注入 InteractionContext
  - onClick / onDblclick / onContextmenu 事件
  - onPointerEnter / onPointerLeave / onPointerMove 指针事件
  - onBeforeUnmount 自动注销事件监听

- ✅ **SPR-023: TSprite 支持 TSpriteMaterial 子组件插槽**
  - 默认插槽 <slot></slot>
  - SpriteContext 提供上下文
  - 子组件自动注入 SpriteContextKey

- ✅ **SPR-024: TSpriteMaterial 精灵材质组件基础实现**
  - 完整的独立 Props 定义
  - 注入 SpriteContext，自动绑定父 Sprite
  - 抛出错误提示：必须在 TSprite 组件内使用

- ✅ **SPR-025: TSpriteMaterial 纹理 map / alphaMap 支持**
  - useTexture 加载外部图片纹理
  - map: string 纹理 URL 配置
  - alphaMap: string 透明度贴图
  - watch 纹理加载完成自动更新 material.map

- ✅ **SPR-026: TSpriteMaterial blending 混合模式支持**
  - blending: BlendingMode 属性
  - watch 监听变更，调用 resolveBlendingMode 映射
  - material.needsUpdate = true 触发重绘

- ✅ **SPR-027: TSpriteMaterial clip 裁剪模式支持**
  - clip: SpriteClipMode ('none' | 'circle' | 'rounded')
  - borderRadius: number 圆角大小
  - tint: 颜色叠加
  - customProgramCacheKey 控制着色器重新编译

- ✅ **SPR-028: TSpriteMaterial 配置变更响应式更新**
  - 所有属性独立 watch 监听
  - color: material.color.set() 实时更新
  - rotation / opacity / transparent / depthTest 等
  - 每项属性变更即时生效，无需重新创建

**新增文件**:

- [packages/vue-three/src/components/TSprite.vue](file:///d:/www/AI/%E9%A1%B9%E7%9B%AE/VueThreeV7/packages/vue-three/src/components/TSprite.vue) - 精灵组件
- [packages/vue-three/src/components/TSpriteMaterial.vue](file:///d:/www/AI/%E9%A1%B9%E7%9B%AE/VueThreeV7/packages/vue-three/src/components/TSpriteMaterial.vue) - 精灵材质组件
- [packages/vue-three/src/index.ts](file:///d:/www/AI/%E9%A1%B9%E7%9B%AE/VueThreeV7/packages/vue-three/src/index.ts) - 新增组件导出
...
## SPR-014 ~ SPR-018: Sprite 着色器特效实现

**完成时间**: 2026-04-29  
**完成内容**:

- ✅ **SPR-017: onBeforeCompile 着色器注入机制**
  - 实现 `applyShaderExtensions` 核心方法
  - material.onBeforeCompile 钩子注入
  - GLSL uniform 变量声明与初始化
  - shader.fragmentShader 字符串替换注入
  - customProgramCacheKey 程序缓存键控制

- ✅ **SPR-014: circle 圆形裁剪着色器扩展**
  - gl_PointCoord 获取精灵纹理坐标
  - length(uv) 计算到中心的距离
  - discard 丢弃圆形范围外的片元
  - uClipMode = 1 启用圆形裁剪

- ✅ **SPR-015: rounded 圆角裁剪着色器扩展**
  - SDF (Signed Distance Field) 算法实现
  - roundedBoxSDF GLSL 内置函数
  - uBorderRadius 圆角半径 uniform 控制
  - uClipMode = 2 启用圆角裁剪

- ✅ **SPR-016: color tint 颜色叠加效果实现**
  - uUseTint 启用标记 uniform
  - uTintColor RGB 颜色向量
  - diffuseColor.rgb \*= uTintColor 叠加着色
  - 支持任意 CSS 颜色值自动转换

- ✅ **SPR-018: sizeAttenuation 透视大小衰减开关**
  - 已在工厂方法 `createSpriteMaterial` 中实现
  - `sizeAttenuation: config.sizeAttenuation ?? true` 默认值
  - `setMaterialConfig` 响应式实时更新
  - Three.js 原生支持，无需着色器修改

**修改文件**:

- [packages/vue-three/src/composables/useSprite.ts](file:///d:/www/AI/%E9%A1%B9%E7%9B%AE/VueThreeV7/packages/vue-three/src/composables/useSprite.ts) - 扩展着色器功能

**着色器注入位置**:

```glsl
// 注入点：#include <map_fragment> 纹理采样之后
...
## SPR-008 ~ SPR-013: useSprite Composable 实现

**完成时间**: 2026-04-29  
**完成内容**:

- ✅ **SPR-008: 实现 useSprite composable 核心逻辑**
  - 遵循现有 useMesh 统一架构模式
  - 自动注入 ThreeContext，抛出合理错误提示
  - 支持可选 config 参数（MaybeRef 响应式配置）
  - shallowRef 管理 Sprite 和 SpriteMaterial 实例
  - 工厂方法 createSprite 创建实例并添加到场景

- ✅ **SPR-009: 提供 SpriteContext 上下文**
  - provide(SpriteContextKey) 提供上下文
  - 包含 sprite 和 setMaterial 两个核心属性
  - 与现有 MeshContext 架构完全一致
  - 支持子组件注入访问

- ✅ **SPR-010: setMaterial 材质切换方法实现**
  - 自动 dispose 旧材质资源
  - 更新 sprite.material 引用
  - 设置 material.needsUpdate = true
  - 完整的资源生命周期管理

- ✅ **SPR-011: 距离可见性控制（minDistance/maxDistance）**
  - computed 检测是否启用距离控制
  - requestAnimationFrame 循环检测相机距离
  - 动态设置 sprite.visible 属性
  - onBeforeUnmount 自动取消动画帧

- ✅ **SPR-012: center 中心点对齐配置实现**
  - 提供 setCenter([x, y]) 方法
  - config 变更自动响应式更新
  - sprite.center.set 原生 Three.js API

- ✅ **SPR-013: Sprite 资源自动清理机制实现**
  - onBeforeUnmount 生命周期钩子
  - 从 scene 中移除 sprite 对象
  - disposeMaterial 清理材质资源
  - 取消距离检测动画帧循环
  - shallowRef 引用置空协助 GC

**修改文件**:

- [packages/vue-three/src/composables/useSprite.ts](file:///d:/www/AI/%E9%A1%B9%E7%9B%AE/VueThreeV7/packages/vue-three/src/composables/useSprite.ts) - 新建 composable
- [packages/vue-three/src/index.ts](file:///d:/www/AI/%E9%A1%B9%E7%9B%AE/VueThreeV7/packages/vue-three/src/index.ts) - 新增导出
...
export function useSprite(config?: MaybeRef<SpriteConfig>) {
  return {
    sprite, // ShallowRef<Sprite> 精灵实例
    setMaterial, // 切换 SpriteMaterial
    setCenter, // 设置中心点 [x, y]
    setMaterialConfig, // 更新材质配置
    updateDistanceVisibility // 手动更新可见性
  }
}
```
...
## SPR-005 ~ SPR-007: 核心工厂扩展

**完成时间**: 2026-04-29  
**完成内容**:

- ✅ **SPR-005: ThreeObjectFactory 添加 createSprite 方法**
  - 调用 createSpriteMaterial 创建材质
  - 实例化 Sprite 对象
  - 通过 applyObject3DConfig 应用通用 3D 对象配置
  - 支持 center 中心点设置
  - 支持 renderOrder 渲染顺序控制
  - 与现有 createMesh / createLine 架构一致

- ✅ **SPR-006: ThreeObjectFactory 添加 createSpriteMaterial 方法**
  - 完整的 SpriteMaterial 实例创建：color / rotation / fog / transparent / opacity
  - 深度测试配置：depthTest / depthWrite
  - 透视控制：sizeAttenuation 大小衰减开关
  - 混合模式：通过 resolveBlendingMode 映射 blending
  - 自定义混合因子：blendSrc / blendDst 支持
  - 用户数据存储：clip / borderRadius / tint / minDistance / maxDistance
  - 所有属性提供合理默认值

- ✅ **SPR-007: resolveBlendingMode 混合模式映射实现**
  - 实现 `resolveBlendingMode` 方法：normal / additive / subtractive / multiply / screen
  - 实现 `resolveBlendingFactor` 辅助方法
  - 将字符串配置映射到 Three.js 原生常量值
  - 提供 Fallback 默认值处理

**修改文件**:

- [packages/vue-three/src/core/factory.ts](file:///d:/www/AI/%E9%A1%B9%E7%9B%AE/VueThreeV7/packages/vue-three/src/core/factory.ts) - 扩展 ThreeObjectFactory

**新增导入**:

- ✅ Sprite / SpriteMaterial 核心类
- ✅ 混合模式常量：NormalBlending / AdditiveBlending / SubtractiveBlending 等
- ✅ 混合因子常量：SrcAlphaFactor / OneFactor / DstColorFactor 等
- ✅ 类型导入：SpriteConfig / SpriteMaterialConfig / BlendingMode / BlendingFactor
...
## SPR-001 ~ SPR-004: Sprite 核心类型与上下文

**完成时间**: 2026-04-29  
**完成内容**:

- ✅ **SPR-001: SpriteConfig 精灵配置类型定义**
  - 继承 Object3DConfig 基础属性
  - 新增 material 精灵材质配置
  - 新增 center?: [number, number] 中心点对齐
  - 新增 renderOrder?: number 渲染顺序控制

- ✅ **SPR-002: SpriteMaterialConfig 材质配置类型定义**
  - 基础渲染属性：color / opacity / transparent
  - 纹理支持：map / alphaMap 支持 URL 或 Texture 实例
  - 渲染配置：rotation / fog / depthTest / depthWrite
  - 透视控制：sizeAttenuation 大小衰减开关
  - 混合模式：blending / blendSrc / blendDst
  - 着色器特效：tint / clip / borderRadius
  - 距离可见性：minDistance / maxDistance

- ✅ **SPR-003: BlendingMode / BlendingFactor 枚举类型**
  - `BlendingMode` 联合类型：normal / additive / subtractive / multiply / screen
  - `BlendingFactor` 联合类型：SrcAlpha / OneMinusSrcAlpha / One / DstColor / OneMinusDstColor
  - `SpriteClipMode` 裁剪模式：none / circle / rounded

- ✅ **SPR-004: SpriteContext 类型与 InjectionKey**
  - 定义 `SpriteContext` 接口：sprite / setMaterial
  - 完整的 JSDoc 文档注释
  - 定义 `SpriteContextKey` InjectionKey 用于依赖注入
  - 与现有上下文架构完全一致

**修改文件**:

- [packages/vue-three/src/types/index.ts](file:///d:/www/AI/%E9%A1%B9%E7%9B%AE/VueThreeV7/packages/vue-three/src/types/index.ts) - 添加 Sprite 类型定义
- [packages/vue-three/src/core/context.ts](file:///d:/www/AI/%E9%A1%B9%E7%9B%AE/VueThreeV7/packages/vue-three/src/core/context.ts) - 添加 SpriteContext

**导出情况**:

- ✅ 所有类型从 index.ts 统一导出
- ✅ `export * from './types'` 自动导出所有类型
- ✅ `export * from './core/context'` 自动导出上下文
...
## DEMO-017 ~ DEMO-023: Playground Sprite ��ʾҳ��
...
- �޲ü� (clip="none") - ���ξ���
- Բ�βü� (clip="circle") - ����Բ��
- Բ�ǲü� (clip="rounded") - ���ֲ�ͬԲ�ǰ뾶
- ʹ�� TSpriteMaterial ��������ģʽ

- **DEMO-020: ���ģʽЧ���Ա���ʾ**
- ���ֻ��ģʽ���ŶԱȣ�normal / additive / multiply / screen
- �ӷ���� (additive) ģ�ⷢ�����Ч��
- ÿ��ģʽʹ�ò�ͬ��ɫչʾЧ������

- **DEMO-021: ����ɼ���/���ؼ���Ⱦ��ʾ**
- sizeAttenuation ���ضԱ���ʾ
- ������͸�Ӵ�С˥��������ԶС��
- �رգ����ؼ��㶨��С�����벻Ӱ�죩
- 8 �������� Z �����У�չʾ����Ч��
...
- [packages/playground/src/views/SpriteDemoView.vue](file:///d:/www/AI/%E9%A1%B9%E7%9B%AE/VueThreeV7/packages/playground/src/views/SpriteDemoView.vue) - Sprite �ۺ���ʾҳ��

**�޸��ļ�**:

- [packages/playground/src/router/index.ts](file:///d:/www/AI/%E9%A1%B9%E7%9B%AE/VueThreeV7/packages/playground/src/router/index.ts) - ���� /demo/sprite ·��
- [packages/vue-three/src/components/TSpriteMaterial.vue](file:///d:/www/AI/%E9%A1%B9%E7%9B%AE/VueThreeV7/packages/vue-three/src/components/TSpriteMaterial.vue) - �޸����ʹ���
