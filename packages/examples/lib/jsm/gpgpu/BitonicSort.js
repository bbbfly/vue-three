...
export const getBitonicFlipIndices = /*@__PURE__*/ Fn(([index, blockHeight]) => {
  ...
  const idx = uvec2(index.mod(halfHeight), blockHeight.sub(index.mod(halfHeight)).sub(1))
...
export const getBitonicDisperseIndices = /*@__PURE__*/ Fn(([index, swapSpan]) => {
  ...
  const idx = uvec2(index.mod(halfHeight), index.mod(halfHeight).add(halfHeight))
...
export class BitonicSort {
  ...
  _getDisperseLocal(readWriteBuffer) {
    ...
    const fnDef = Fn(() => {
      ...
      Loop(
        { start: localBlockHeight, end: uint(1), type: 'uint', condition: '>', update: '>>= 1' },
        () => {
          // Ensure that last dispatch op executed
          workgroupBarrier()
