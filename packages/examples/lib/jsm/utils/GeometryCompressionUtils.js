...
function compressNormals(geometry, encodeMethod) {
  ...
  if (!normal) {
    console.error(
      'THREE.GeometryCompressionUtils.compressNormals(): Geometry must contain normal attribute.'
    )
  }
...
function compressUvs(geometry) {
  ...
  if (!uvs) {
    console.error(
      'THREE.GeometryCompressionUtils.compressUvs(): Geometry must contain uv attribute.'
    )
  }
