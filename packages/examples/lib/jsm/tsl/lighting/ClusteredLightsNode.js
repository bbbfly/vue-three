...
class ClusteredLightsNode extends LightsNode {
  ...
  setupLights(builder, lightNodes) {
    ...
    Fn(() => {
      Loop(this.maxLightsPerCluster, ({ i }) => {
        ...
        If(
          distance.equal(0).or(dot(lightVector, lightVector).lessThanEqual(distance.mul(distance))),
          () => {
  ...
  create(width, height) {
    ...
    const lightsTexture = new DataTexture(
      lightsData,
      lightsData.length / 8,
      2,
      RGBAFormat,
    ...
    const compute = Fn(() => {
      ...
      const ndcXmin = float(cx)
        .mul(2.0 / NX)
        .sub(1.0)
      const ndcXmax = float(cx.add(int(1)))
        .mul(2.0 / NX)
        .sub(1.0)
      ...
      const zFarCluster = this._cameraNear
