...
class SpotLightDataNode extends Node {
  ...
  constructor(maxCount = 16) {
    ...
    this.positionsAndCutoffNode = uniformArray(this._positionsAndCutoff, 'vec4').setGroup(
      renderGroup
    )
    this.directionsAndDecayNode = uniformArray(this._directionsAndDecay, 'vec4').setGroup(
      renderGroup
    )
  ...
  setLights(lights) {
    if (lights.length > this.maxCount) {
      warn(
        `${lights.length} lights exceed the configured max of ${this.maxCount}. Excess lights are ignored.`
      )
    }
  ...
  setup(builder) {
    ...
    Loop(this.countNode, ({ i }) => {
      ...
      const lightColor = this.colorsNode
        .element(i)
        .mul(spotAttenuation)
