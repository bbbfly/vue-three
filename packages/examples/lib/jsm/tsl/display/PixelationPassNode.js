...
class PixelationNode extends TempNode {
  ...
  setup() {
    ...
    const sampleDepth = (x, y) =>
      depthNode.sample(uvNodeDepth.add(vec2(x, y).mul(this._resolution.zw))).r
    ...
    const pixelation = Fn(() => {
      ...
      If(
        this.depthEdgeStrength.greaterThan(0.0).or(this.normalEdgeStrength.greaterThan(0.0)),
        () => {
      ...
      const strength = dei
        .greaterThan(0)
        .select(
          float(1.0).sub(dei.mul(this.depthEdgeStrength)),
          nei.mul(this.normalEdgeStrength).add(1)
        )
