...
class FSR1Node extends TempNode {
  ...
  setup(builder) {
    ...
    const easu = Fn(() => {
      ...
      const rDirLen = float(1.0)
        .div(sqrt(max(dirSq, float(1.0 / 32768.0))))
        .toConst()
      ...
      const lob = float(0.5)
        .add(float(1.0 / 4.0 - 0.04 - 0.5).mul(len))
        .toConst()
    ...
    const rcas = Fn(() => {
      ...
      const p = ivec2(
        int(floor(targetUV.x.mul(texSize.x))),
        int(floor(targetUV.y.mul(texSize.y)))
      ).toConst()
 ...
 * @returns {FSR1Node}
 */
export const fsr1 = (node, sharpness, denoise) =>
  new FSR1Node(convertToTexture(node), sharpness, denoise)
