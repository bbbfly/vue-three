...
class BilateralBlurNode extends TempNode {
  ...
  setup(builder) {
    ...
    const blur = Fn(() => {
      ...
      const colorSigmaFactor = float(-0.5)
        .div(float(this.sigmaColor * this.sigmaColor))
        .toConst()
 ...
 */
export const bilateralBlur = (node, directionNode, sigma, sigmaColor) =>
  new BilateralBlurNode(convertToTexture(node), directionNode, sigma, sigmaColor)
