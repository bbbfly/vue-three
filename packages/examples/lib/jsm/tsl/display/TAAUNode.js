...
class TAAUNode extends TempNode {
  ...
  updateBefore(frame) {
    ...
    const srcW =
      currentDepth.image !== null && currentDepth.image !== undefined ? currentDepth.image.width : 0
    const srcH =
      currentDepth.image !== null && currentDepth.image !== undefined
        ? currentDepth.image.height
        : 0

    if (srcW > 0 && srcH > 0) {
      if (
        this._previousDepthRenderTarget.width !== srcW ||
        this._previousDepthRenderTarget.height !== srcH
      ) {
...
const _haltonOffsets = /*@__PURE__*/ Array.from({ length: 32 }, (_, index) => [
  _halton(index + 1, 2),
  _halton(index + 1, 3)
])
 ...
 * @returns {TAAUNode}
 */
export const taau = (beautyNode, depthNode, velocityNode, camera) =>
  new TAAUNode(convertToTexture(beautyNode), depthNode, velocityNode, camera)
