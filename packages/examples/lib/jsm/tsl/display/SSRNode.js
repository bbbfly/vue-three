...
class SSRNode extends TempNode {
  ...
  setup(builder) {
    ...
    const ssr = Fn(() => {
      ...
      If(
        this._isPerspectiveCamera.and(d1viewPosition.z.greaterThan(this._cameraNear.negate())),
        () => {
      ...
      const d1 = getScreenPosition(d1viewPosition, this._cameraProjectionMatrix)
      ...
      Loop(totalStep, ({ i }) => {
        ...
        If(viewReflectRayZ.lessThanEqual(vZ), () => {
          ...
          const vPNeighbor = getViewPosition(
            uvNeighbor,
            d,
            this._cameraProjectionMatrixInverse
          ).toVar()
    ...
    this._blurMaterial.fragmentNode = boxBlur(reflectionBuffer, {
