    ...
    const horizonSampling = Fn(
      ([
        ...
        If(this.useScreenSpaceSampling.equal(true), () => {
          ...
          stepRadius.assign(
            max(RADIUS.mul(this._halfProjScale).div(viewPosition.z.negate()), float(STEP_COUNT))
          ) // Port note: viewZ is negative so a negate is required
        })
        ...
        Loop({ start: uint(0), end: STEP_COUNT, type: 'uint', condition: '<' }, ({ i }) => {
          ...
          const linearThicknessMultiplier = this.useLinearThickness
            .equal(true)
            .select(sampleViewPosition.z.negate().div(this._cameraFar).clamp().mul(100), float(1))
          ...
          frontBackHorizon = clamp(
            div(mul(samplingDirection, frontBackHorizon.negate()).sub(n.sub(HALF_PI)), PI)
