...
class SSSNode extends TempNode {
  ...
  constructor(depthNode, camera, mainLight) {
     ...
     * @type {RenderTarget}
     */
    this._sssRenderTarget = new RenderTarget(1, 1, {
      depthBuffer: false,
      format: RedFormat,
  ...
  setup(builder) {
    ...
    const sss = Fn(() => {
      ...
      ).toVar('rayStartPosition')
      const rayDirection = this._cameraViewMatrix
        .transformDirection(
          lightPosition(this._mainLight).sub(lightTargetPosition(this._mainLight))
        )
        .toConst('rayDirection')
      const rayEndPosition = rayStartPosition
        .add(rayDirection.mul(this.maxDistance))
        .toConst('rayEndPosition')

      // d0 and d1 are the start and maximum points of the ray in screen space
      const d0 = screenCoordinate.xy.toVar()
      const d1 = getScreenPosition(rayEndPosition, this._cameraProjectionMatrix)
        .mul(this._resolution)
        .toVar()
      ...
      const offset = fract(noise.add(this._temporalOffset))
        .add(rand(uvNode.add(this._frameId)))
        .toConst('offset')
      ...
      Loop(totalStep, ({ i }) => {
        ...
        const xy = vec2(
          d0.x.add(xSpan.mul(float(i).add(offset))),
          d0.y.add(ySpan.mul(float(i).add(offset)))
        ).toVar()
