...
function createMatrixPair(leftSpec, rightSpec) {
  ...
  function specToColumnMajor(spec) {
    ...
    return [
      r[0],
      g[0],
      b[0], // Column 0: coefficients for input R
      r[1],
      g[1],
      b[1], // Column 1: coefficients for input G
      r[2],
      g[2],
      b[2] // Column 2: coefficients for input B
    ]
  }
...
class AnaglyphPassNode extends StereoCompositePassNode {
  ...
  updateStereoCamera(coordinateSystem) {
    ...
    stereo.cameraL.matrixWorld.compose(
      stereo.cameraL.position,
      stereo.cameraL.quaternion,
      stereo.cameraL.scale
    )
    ...
    stereo.cameraR.matrixWorld.compose(
      stereo.cameraR.position,
      stereo.cameraR.quaternion,
      stereo.cameraR.scale
    )
  ...
  setup(builder) {
    ...
    const anaglyph = Fn(() => {
      ...
      const color = clamp(
        this._colorMatrixLeft.mul(colorL.rgb).add(this._colorMatrixRight.mul(colorR.rgb))
      )
