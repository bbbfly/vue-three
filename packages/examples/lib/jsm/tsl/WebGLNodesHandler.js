import { GLSL3, UniformsGroup, Compatibility, Color, UniformsLib, UniformsUtils } from 'three'
import {
  context,
  cubeTexture,
  reference,
...
export class WebGLNodesHandler {
  ...
  renderStart(scene, camera) {
    ...
    // traversal and build
    const { customProgramCacheKeyCallback, onBeforeRenderCallback } = this
  ...
  build(material, object, parameters) {
    const { nodeFrame, renderer, getOutputCallback, onDisposeMaterialCallback, renderStack } = this

    const { camera, sceneContext } = renderStack[renderStack.length - 1]

    const { fogNode, environmentNode, lightsNode, scene } = sceneContext
