    ...
    if (this.uniforms.length > 0) {
      ...
      for (const uniform of this.uniforms) {
        ...
        if (uniform.type.includes('texture')) {
          textureGlobals.push(
            `@group(${this.groupIndex}) @binding(${bindingIndex++}) var ${uniform.name}: ${this.getWgslType(uniform.type)};`
          )
          textureGlobals.push(
            `@group(${this.groupIndex}) @binding(${bindingIndex++}) var ${uniform.name}_sampler: sampler;`
          )
        } else {
