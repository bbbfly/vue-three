    ...
    return new Proxy(result, {
      get(target, prop) {
        ...
        if (typeof prop === 'string' && /^\d+$/.test(prop)) {
          if (!warned) {
            console.warn(
              'THREE.VOXLoader: Accessing result as an array is deprecated. Use result.chunks[] instead.'
            )
            warned = true
          }
        ...
        if (prop === Symbol.iterator) {
          if (!warned) {
            console.warn(
              'THREE.VOXLoader: Iterating result as an array is deprecated. Use result.chunks instead.'
            )
            warned = true
          }
    ...
    for (let slice = 0; slice <= dimsD; slice++) {
      ...
      for (let vv = 0; vv < dimsV; vv++) {
        for (let uu = 0; uu < dimsU; uu++) {
          ...
          const behind =
            slice > 0 ? volume[x0 - q[0] + (y0 - q[1]) * sx + (z0 - q[2]) * sx * sy] : 0
      ...
      for (let vv = 0; vv < dimsV; vv++) {
        for (let uu = 0; uu < dimsU; ) {
          ...
          if (c !== 0) {
            ...
            const toThree = p => [p[0] - sx / 2, p[2] - sz / 2, -p[1] + sy / 2]
