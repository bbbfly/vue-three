...
class SVGLoader extends Loader {
    ...
    function parseGradients(xml) {
      ...
      for (const node of gradientNodes) {
        ...
        if (stopNodes.length > 0) {
          ...
          for (const s of stopNodes) {
            ...
            opacity =
              opacity === null || opacity === '' || opacity === undefined
                ? 1
      ...
      for (const id in parsed) {
        ...
        const gradient = {
          ...
          gradientUnits: units,
          spreadMethod:
            a.spreadMethod === 'reflect' || a.spreadMethod === 'repeat' ? a.spreadMethod : 'pad',
  ...
  static createShapes(shapePath) {
    ...
    let fillRule =
      (shapePath.userData && shapePath.userData.style && shapePath.userData.style.fillRule) ||
      'nonzero'

    if (fillRule !== 'nonzero' && fillRule !== 'evenodd') {
      console.warn(
        'THREE.SVGLoader: fill-rule "' + fillRule + '" is not supported, falling back to "nonzero".'
      )
      fillRule = 'nonzero'
    }
      ...
      if (nextPoint !== undefined) {
        ...
        if (dot > Number.EPSILON) {
          ...
          if (innerSideModified) {
            ...
            const foldCross =
              (outerPoint.x - refPt.x) * (innerPoint.y - refPt.y) -
...
function buildGradientTexture(gradient, shapePath, resolution = 256) {
  ...
  if (gradient.type === 'linearGradient') {
    ...
    // M * (vx, vy, 1) = (t, 0.5, 1)
    textureMatrix = new Matrix3().set(a, b, c, 0, 0, 0.5, 0, 0, 1)
  } else {
    ...
    if (isBBoxUnits) {
      ...
      r =
        r * Math.sqrt((localBBox.width * localBBox.width + localBBox.height * localBBox.height) / 2)
    }
