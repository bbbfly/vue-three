...
class USDCParser {
  ...
  _readFieldSets() {
    ...
    if (this.version.major === 0 && this.version.minor < 4) {
      ...
      const indices = decompressIntegers32(
        compressed.buffer.slice(compressed.byteOffset, compressed.byteOffset + compressedSize),
        numFieldSets
      )
  ...
  _readValue(valueRep) {
    ...
    if (offset < 0 || offset >= this.buffer.byteLength) {
      throw new RangeError(
        'USDCParser: Invalid payload offset ' + offset + ' for type ' + type + '.'
      )
    }
  ...
  _readTimeSamples(valueRep) {
    ...
    const timesArray =
      times instanceof Float64Array ? Array.from(times) : Array.isArray(times) ? times : [times]
