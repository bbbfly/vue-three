...
class USDComposer {
      ...
      if (spec.specType === SpecType.Prim) {
        ...
        if (typeName === 'Shader' && lastSlash > 0) {
          ...
          while (ancestorPath.length > 0) {
            ...
            if (
              ancestorSpec &&
              ancestorSpec.specType === SpecType.Prim &&
              ancestorSpec.fields.typeName === 'Material'
            ) {
  ...
  _buildMesh(path, spec) {
    ...
    const hasSkinning =
      jointIndices && jointWeights && jointIndices.length > 0 && jointWeights.length > 0
  ...
  _buildCamera(path) {
    ...
    const focusDistance = this._parseNumber(
      attrs['focusDistance'],
      USD_CAMERA_DEFAULTS.focusDistance
    )
  ...
  _buildGeometry(path, fields, hasSkinning = false) {
    ...
    if (normals && normals.length > 0) {
      ...
      geometry.setAttribute(
        'normal',
        new BufferAttribute(new Float32Array(this._expandAttribute(vertexNormals, indices, 3)), 3)
      )
    }
  ...
  _buildGeometryWithSubsets(fields, geomSubsets, hasSkinning = false) {
    ...
    const { indices: origIndices, pattern: triPattern } = this._triangulateIndicesWithPattern(
      faceVertexIndices,
      faceVertexCounts,
      points,
      holeMap
    ...
    const faceVaryingIdentity =
      (uvs && !uvIndices && uvs.length / 2 === numFaceVertices) ||
    ...
    const vertexNormals =
      !normals && origIndices.length > 0 ? this._computeVertexNormals(points, origIndices) : null
  ...
  _findUVPrimvar(fields) {
    for (const key in fields) {
      ...
      if (key.endsWith(':typeName') || key.endsWith(':elementSize') || key.endsWith(':indices'))
        continue
  ...
  _triangulateNGon(faceIndices, points) {
    ...
    for (const idx of faceIndices) {
      contour3D.push(new Vector3(points[idx * 3], points[idx * 3 + 1], points[idx * 3 + 2]))
    }
  ...
  _triangulateNGonWithHoles(outerIndices, holeContours, points) {
    ...
    for (const idx of outerIndices) {
      outer3D.push(new Vector3(points[idx * 3], points[idx * 3 + 1], points[idx * 3 + 2]))
    }
    ...
    for (const holeIndices of holeContours) {
      ...
      for (const idx of holeIndices) {
        const p = new Vector3(points[idx * 3], points[idx * 3 + 1], points[idx * 3 + 2])
  ...
  _applyPreviewSurface(material, shaderPath) {
    ...
    // Emissive
    applyTexture('inputs:emissiveColor', 'emissiveMap', SRGBColorSpace, color => {
      if (Array.isArray(color) && color.length >= 3) {
    ...
    // Metallic
    const hasMetalnessMap = applyTexture('inputs:metallic', 'metalnessMap', NoColorSpace, value => {
    ...
    // Specular color
    applyTexture('inputs:specularColor', 'specularColorMap', SRGBColorSpace, color => {
      if (Array.isArray(color) && color.length >= 3) {
    ...
    const opacityThreshold =
      fields['inputs:opacityThreshold'] !== undefined ? fields['inputs:opacityThreshold'] : 0.0
  ...
  _applyOpenPBRSurface(material, shaderPath) {
    ...
    // Base metalness
    applyTexture('inputs:base_metalness', 'metalnessMap', NoColorSpace, value => {
    ...
    // Specular roughness
    applyTexture('inputs:specular_roughness', 'roughnessMap', NoColorSpace, value => {
      if (typeof value === 'number') {
        material.roughness = value
      }
    })
