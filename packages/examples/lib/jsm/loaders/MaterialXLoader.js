...
const MXElements = [
  ...
  new MXElement('fractal3d', mx_fractal_noise_float, [
    'position',
    'octaves',
    'lacunarity',
    'diminish',
  ...
  new MXElement('place2d', mx_place2d, [
    'texcoord',
    'pivot',
    'scale',
    'rotate',
...
class MaterialXNode {
  constructor(materialX, nodeXML, nodePath = '') {
    if (!materialX || typeof materialX !== 'object') {
      console.warn('MaterialXNode: materialX argument is not an object!', {
        materialX,
        nodeXML,
        nodePath
      })
  ...
  getNode(out = null) {
    ...
    if (
      (this.element === 'separate3' ||
        this.element === 'separate2' ||
        this.element === 'separate4') &&
      out &&
      typeof out === 'string' &&
      out.startsWith('out')
    ) {
    ...
    if (this.isConst) {
      ...
      if (element === 'convert') {
        ...
        if (!nodeElement.nodeFunc) {
          throw new Error(
            `THREE.MaterialXLoader: Unexpected node 2 ${new XMLSerializer().serializeToString(this.nodeXML)}.`
          )
        }
  ...
  setStandardSurfaceToGltfPBR(material) {
    ...
    if (
      ...
      thinFilmThicknessNode.value !== undefined &&
      thinFilmThicknessNode.value > 0
    ) {
