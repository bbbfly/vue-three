...
class ColladaComposer {
  ...
  buildTranslateTrack(object3D, channelData, transformInfo, tracks) {
    if (channelData.default && channelData.default.stride === 3) {
      ...
      const track = new VectorKeyframeTrack(object3D.uuid + '.position', times, values)
  ...
  buildRotateTrack(object3D, sid, channelData, transformInfo, nodeData, tracks) {
    ...
    const track = new QuaternionKeyframeTrack(object3D.uuid + '.quaternion', times, values)
  ...
  buildScaleTrack(object3D, channelData, transformInfo, tracks) {
    if (channelData.default && channelData.default.stride === 3) {
      ...
      const track = new VectorKeyframeTrack(object3D.uuid + '.scale', times, values)
  ...
  evaluateBezierComponent(componentData, i0, i1, t0, t1, time) {
    ...
    for (let iter = 0; iter < 8; iter++) {
      ...
      const dbx =
        3 * oneMinusS2 * (c0x - t0) + 6 * oneMinusS * s * (c1x - c0x) + 3 * s2 * (t1 - c1x)
      ...
      if (image !== null) {
        ...
        if (loader !== undefined) {
          ...
          if (
            ...
            self.isEmpty(extra.technique) === false
          ) {
  ...
  buildHierarchyTranslateTrack(transformNode, channelData, transformInfo, tracks) {
    ...
    const track = new VectorKeyframeTrack(transformNode.uuid + '.position', times, values)
  ...
  buildHierarchyRotateTrack(transformNode, channelData, transformInfo, tracks) {
    ...
    const axis =
      transformNode.userData.rotationAxis ||
    ...
    const track = new QuaternionKeyframeTrack(transformNode.uuid + '.quaternion', times, values)
  ...
  buildHierarchyScaleTrack(transformNode, channelData, transformInfo, tracks) {
    ...
    const track = new VectorKeyframeTrack(transformNode.uuid + '.scale', times, values)
