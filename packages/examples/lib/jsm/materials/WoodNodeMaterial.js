...
const spaceWarp = TSL.Fn(([p, warpStrength, xyScale, zScale]) => {
  ...
  const noise = noiseFbm3d(
    combinedXyz.mul(1.6 * 1.5),
    TSL.float(1),
    TSL.float(0.5),
    TSL.float(2),
    TSL.int(1)
...
const woodRings = TSL.Fn(
  ([w, ringThickness, ringBias, ringSizeVariance, ringVarianceScale, barkThickness]) => {
    const rings = noiseFbm(
      w.mul(ringVarianceScale),
      TSL.float(1),
      TSL.float(0.5),
      TSL.float(1),
      TSL.int(1)
    ...
    const sharpRings = TSL.min(
      mapRange(rings, 0, ringBias, 0, 1, TSL.bool(true)),
      mapRange(rings, ringBias, 1, 1, 0, TSL.bool(true))
    )

    const blurAmount = TSL.max(TSL.positionView.length().div(10), 1)
    const blurredRings = TSL.smoothstep(blurAmount.negate(), blurAmount, sharpRings.sub(0.5))
      .mul(0.5)
      .add(0.5)
...
const woodDetail = TSL.Fn(([warp, p, y, splotchScale]) => {
  const radialCoords = TSL.clamp(TSL.atan(warp.y, warp.x).div(TSL.PI2).add(0.5), 0, 1).mul(
    TSL.PI2.mul(3)
  )
...
const wood = TSL.Fn(
  ([
    ...
    const cells = cellStructure(
      mainWarp,
      cellScale,
      cellSize.div(TSL.max(TSL.positionView.length().mul(10), 1))
    )
...
uniforms.centerSize = TSL.uniform(params.centerSize).onObjectUpdate(
  ({ material }) => material.centerSize
)
uniforms.largeWarpScale = TSL.uniform(params.largeWarpScale).onObjectUpdate(
  ({ material }) => material.largeWarpScale
)
uniforms.largeGrainStretch = TSL.uniform(params.largeGrainStretch).onObjectUpdate(
  ({ material }) => material.largeGrainStretch
)
uniforms.smallWarpStrength = TSL.uniform(params.smallWarpStrength).onObjectUpdate(
  ({ material }) => material.smallWarpStrength
)
uniforms.smallWarpScale = TSL.uniform(params.smallWarpScale).onObjectUpdate(
  ({ material }) => material.smallWarpScale
)
uniforms.fineWarpStrength = TSL.uniform(params.fineWarpStrength).onObjectUpdate(
  ({ material }) => material.fineWarpStrength
)
uniforms.fineWarpScale = TSL.uniform(params.fineWarpScale).onObjectUpdate(
  ({ material }) => material.fineWarpScale
)
uniforms.ringThickness = TSL.uniform(params.ringThickness).onObjectUpdate(
  ({ material }) => material.ringThickness
)
uniforms.ringBias = TSL.uniform(params.ringBias).onObjectUpdate(({ material }) => material.ringBias)
uniforms.ringSizeVariance = TSL.uniform(params.ringSizeVariance).onObjectUpdate(
  ({ material }) => material.ringSizeVariance
)
uniforms.ringVarianceScale = TSL.uniform(params.ringVarianceScale).onObjectUpdate(
  ({ material }) => material.ringVarianceScale
)
uniforms.barkThickness = TSL.uniform(params.barkThickness).onObjectUpdate(
  ({ material }) => material.barkThickness
)
uniforms.splotchScale = TSL.uniform(params.splotchScale).onObjectUpdate(
  ({ material }) => material.splotchScale
)
uniforms.splotchIntensity = TSL.uniform(params.splotchIntensity).onObjectUpdate(
  ({ material }) => material.splotchIntensity
)
uniforms.cellScale = TSL.uniform(params.cellScale).onObjectUpdate(
  ({ material }) => material.cellScale
)
uniforms.cellSize = TSL.uniform(params.cellSize).onObjectUpdate(({ material }) => material.cellSize)
uniforms.darkGrainColor = TSL.uniform(new THREE.Color(params.darkGrainColor)).onObjectUpdate(
  ({ material }, self) => self.value.set(material.darkGrainColor)
)
uniforms.lightGrainColor = TSL.uniform(new THREE.Color(params.lightGrainColor)).onObjectUpdate(
  ({ material }, self) => self.value.set(material.lightGrainColor)
)
uniforms.transformationMatrix = TSL.uniform(
  new THREE.Matrix4().copy(params.transformationMatrix)
).onObjectUpdate(({ material }) => material.transformationMatrix)
