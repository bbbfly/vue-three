...
export class Profiler extends EventDispatcher {
  ...
  getSize() {
    if (
      this.panel.classList.contains('visible') === false ||
      this.panel.classList.contains('no-tabs')
    ) {
...
`
  ...
  addBuiltinTab(tab) {
    ...
    builtinButton.onclick = e => {
      ...
      const isCurrentlyActive =
        miniContent.style.display !== 'none' && miniContent.children.length > 0
    ...
    if (!tab.isVisible) {
      ...
      const hasVisibleBuiltinButtons = Array.from(
        this.builtinTabsContainer.querySelectorAll('.builtin-tab-btn')
  ...
  createDetachedWindow(tab, x, y) {
    ...
    this.setupDetachedWindowResize(
      windowPanel,
      resizerTop,
      resizerRight,
      resizerBottom,
  ...
  setupDetachedWindowDrag(windowPanel, header, tab) {
    ...
    const onDragMove = e => {
      ...
      const isOverPanel =
        ...
        currentX <= panelRect.right &&
        currentY >= panelRect.top &&
        currentY <= panelRect.bottom
