...
class Timeline extends Tab {
  ...
  startRecording() {
    ...
    const methods = Object.getOwnPropertyNames(Object.getPrototypeOf(backend)).filter(
        ...
        backend[prop] = (...args) => {
          ...
          if (!this.currentFrame || this.currentFrame.id !== frameNumber) {
            if (this.currentFrame) {
              ...
              const normalizedTriangles =
                this.baseTriangles > 0 ? (t / this.baseTriangles) * TRIANGLES_GRAPH_LIMIT : 0
    ...
    switch (method) {
      ...
      case 'beginRender': {
        ...
        if (
          ...
          !renderContext.renderTarget.isPostProcessingRenderTarget
        ) {
      ...
      case 'createRenderPipeline': {
        ...
        const details = {
          object: renderObject.object
            ? renderObject.object.name || renderObject.object.type || 'unknown'
            : 'unknown',
          material: renderObject.material
            ? renderObject.material.name || renderObject.material.type || 'unknown'
            : 'unknown'
        }
  ...
  formatDetails(details) {
    ...
    for (const key in details) {
      if (details[key] !== undefined) {
        parts.push(
          `<span style="opacity: 0.5">${key}:</span> <span style="color: var(--text-secondary); opacity: 1">${details[key]}</span>`
        )
      }
    }
  ...
  selectFrame(index) {
    ...
    // Update UI texts
    const group = (c, text) =>
      `<span style="display:inline-flex;align-items:center;margin-left:12px;"><span style="width:6px;height:6px;border-radius:50%;background-color:${c};margin-right:6px;"></span>${text}</span>`
    const maxTriangles = Math.max(this.baseTriangles, frame.triangles || 0)
    this.frameInfo.innerHTML =
      'Frame: ' +
      frame.id +
      group('var(--color-fps)', (frame.fps || 0).toFixed(1) + ' FPS') +
      group('var(--color-call)', frame.calls.length + ' calls') +
      group('var(--color-red)', (frame.triangles || 0) + ' / ' + maxTriangles + ' triangles')
  ...
  renderTimelineTrack(frame) {
    ...
    if (this.isHierarchicalView) {
      ...
      for (let i = 0; i < frame.calls.length; i++) {
        ...
        if (
          currentGroup &&
          currentGroup.method === call.method &&
          currentGroup.formatedDetails === formatedDetails &&
          !isStructural
        ) {
          currentGroup.count++
        } else {
      ...
      const elementStack = [
        { element: this.timelineTrack, isCollapsed: false, id: '', beginCount: 0 }
      ]
      ...
      for (let i = 0; i < groupedCalls.length; i++) {
        ...
        if (call.method.startsWith('begin')) {
          ...
          block.titleSpan.innerHTML =
            call.method +
            (call.formatedDetails ? call.formatedDetails : '') +
            (call.count > 1 ? ` <span style="opacity: 0.5">( ${call.count} )</span>` : '')
