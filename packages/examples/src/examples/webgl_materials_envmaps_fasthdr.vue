  ...
  <TCanvas ref="canvasRef" antialias @animate="animate">
    <TScene ref="sceneRef">
      ...
      <TMesh :position="[0, 0, 2]">
        ...
        <TMeshPhysicalMaterial :transmission="1.0" :thickness="2.0" :metalness="0.0" :roughness="0.0" />
      </TMesh>
...
import { useGui, TCanvas, TPerspectiveCamera, TOrbitControls, TMesh, TSphere, TMeshPhysicalMaterial, TMeshStandardMaterial, TScene } from '@vue-three/vue-three'
...
function initScene() {
  ktx2Loader = new KTX2Loader()
    ...
    .detectSupport(renderer.value)

  loadTexture(params.value.image)

  gui.add(params.value, 'image', imageOptions).name('Image').onChange(() => {
    loadTexture(params.value.image)
  })

  gui.add(params.value, 'exposure', 0, 2, 0.01).name('Exposure')

  gui.add(params.value, 'fov', 10, 100).name('FOV').onChange(() => {
    if (camera.value) {
...
watch([canvasRef, sceneRef], async () => {
  await nextTick()
