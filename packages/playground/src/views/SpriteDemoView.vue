<template>
  <MainLayout>
    ...
    <template #panel>
      <div class="p-4">
        <h2 class="text-lg font-bold mb-4 text-white">Sprite 精灵演示</h2>

        <div class="mb-6 p-3 bg-gray-800 rounded-lg">
          <h3 class="text-sm font-semibold text-yellow-400 mb-2">点击状态</h3>
          <p class="text-xs text-gray-300">精灵1: {{ spriteClicked ? '已点击 ✓' : '未点击' }}</p>
          <p class="text-xs text-gray-300">精灵2: {{ sprite2Clicked ? '已点击 ✓' : '未点击' }}</p>
          <p class="text-xs text-gray-300">悬停: {{ isHovering ? '是' : '否' }}</p>
          <p v-if="lastClickedPOI" class="text-xs text-green-400 mt-2">
            最后点击 POI: {{ lastClickedPOI.name }}
          </p>
        </div>

        <div class="mb-6 p-3 bg-gray-800 rounded-lg">
          <h3 class="text-sm font-semibold text-blue-400 mb-2">精灵属性实时调试</h3>

          <div class="space-y-3">
            <div>
              <label class="text-xs text-gray-400 block mb-1"
                >缩放: {{ debugSpriteScale.toFixed(2) }}</label
              >
              <input
                v-model="debugSpriteScale"
                type="range"
                min="0.2"
                max="2"
                step="0.1"
            ...
            <div>
              <label class="text-xs text-gray-400 block mb-1"
                >透明度: {{ debugSpriteOpacity.toFixed(2) }}</label
              >
              <input
                v-model="debugSpriteOpacity"
                type="range"
                min="0"
                max="1"
                step="0.05"
            ...
            <div>
              <label class="text-xs text-gray-400 block mb-1"
                >圆角半径: {{ debugBorderRadius.toFixed(2) }}</label
              >
              <input
                v-model="debugBorderRadius"
                type="range"
                min="0"
                max="0.5"
                step="0.05"
            ...
            <div>
              <label class="text-xs text-gray-400 block mb-1">裁剪模式</label>
              <select
                v-model="debugSpriteClip"
                class="w-full bg-gray-700 text-white text-xs p-1 rounded"
              >
            ...
            <div>
              <label class="text-xs text-gray-400 block mb-1">混合模式</label>
              <select
                v-model="debugSpriteBlending"
                class="w-full bg-gray-700 text-white text-xs p-1 rounded"
              >
            ...
            <div>
              <label class="text-xs text-gray-400 block mb-1">颜色</label>
              <input v-model="debugSpriteColor" type="color" class="w-full h-8" />
            </div>
            <div class="flex items-center gap-2">
              <input id="sizeAtten" v-model="debugSizeAttenuation" type="checkbox" />
              <label for="sizeAtten" class="text-xs text-gray-400">透视大小衰减</label>
            </div>
          </div>
        </div>
...
const debugSpriteBlending = ref<any>(undefined)
