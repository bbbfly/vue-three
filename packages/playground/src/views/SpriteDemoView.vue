    ...
    <template #viewport>
      <TCanvas antialias alpha @click="handleCanvasClick">
        <TScene background="#1a1a2e">
          ...
          <TMesh v-for="i in 5" :key="'cube-' + i" :position="[(i - 3) * 2.5, 0.75, -2]" :cast-shadow="true"
            :receive-shadow="true">
          ...
          <div class="sprite-demo-section">
            ...
            <TSprite ref="basicSpriteRef" :position="[-8, 2, -4]" :scale="[1, 1, 1]" color="#e74c3c" />
          ...
          <div class="sprite-demo-section">
            ...
            <TSprite v-for="(poi, index) in poiMarkers" :key="'poi-' + index" ref="poiSpriteRefs"
              :position="poi.position" :scale="[0.6, 0.6, 0.6]" :color="poi.color" clip="circle" :render-order="100"
              @click="handlePOIClick(poi)" />
            <TSprite v-for="(poi, index) in poiMarkers" :key="'poi-glow-' + index" :position="poi.position"
              :scale="[1, 1, 1]" :color="poi.color" clip="circle" blending="additive" :opacity="0.3" />
          </div>
          ...
          <div class="sprite-demo-section">
            ...
            <TSprite v-for="(blend, index) in blendingModes" :key="'blend-' + index" :position="[-6 + index * 2, 2, 3]"
              :scale="[1.2, 1.2, 1.2]" :color="blend.color">
              <TSpriteMaterial :blending="blend.mode" :color="blend.color" clip="circle" :opacity="blend.opacity" />
            </TSprite>
          </div>

          <div class="sprite-demo-section">
            <h3 class="demo-title">像素级渲染 (sizeAttenuation)</h3>
            <TSprite ref="attenuationOnRef" :position="[2, 2, 3]" :scale="[0.5, 0.5, 0.5]" color="#e67e22"
              :size-attenuation="true" />
            <TSprite ref="attenuationOffRef" :position="[4, 2, 3]" :scale="[0.5, 0.5, 0.5]" color="#16a085"
              :size-attenuation="false" />
            <TSprite v-for="i in 8" :key="'distance-' + i" :position="[i * 1.2 - 5, 1.5, 6 + i * 0.5]"
              :scale="[0.4, 0.4, 0.4]" :color="getDistanceColor(i)" :size-attenuation="false" clip="circle" />
          </div>

          <div class="sprite-demo-section">
            <h3 class="demo-title">可交互精灵 (点击测试)</h3>
            <TSprite ref="clickableSpriteRef" :position="[6, 2.5, -4]" :scale="[1.2, 1.2, 1.2]"
              :color="spriteClicked ? '#2ecc71' : '#e74c3c'" clip="circle" @click="handleSpriteClick"
              @pointer-over="handlePointerOver" @pointer-out="handlePointerOut" />
            <TSprite :position="[8, 2.5, -4]" :scale="[1.2, 1.2, 1.2]" :color="sprite2Clicked ? '#2ecc71' : '#3498db'"
              clip="rounded" :border-radius="0.2" @click="handleSprite2Click" />
          </div>

          <div class="sprite-demo-section">
            <h3 class="demo-title">动态属性调试</h3>
            <TSprite ref="debugSpriteRef" :position="[6, 2, 0]"
              :scale="[debugSpriteScale, debugSpriteScale, debugSpriteScale]" :color="debugSpriteColor"
              :opacity="debugSpriteOpacity" :clip="debugSpriteClip" :border-radius="debugBorderRadius"
              :blending="debugSpriteBlending" :size-attenuation="debugSizeAttenuation" />
          </div>
        </TScene>
      </TCanvas>
    </template>
    <template #properties>
      <div class="p-4">
        ...
        <div class="mb-6 p-3 bg-gray-50 rounded-lg border border-gray-200">
          ...
          <div class="space-y-3">
            <div>
              <label class="text-xs text-gray-500 block mb-1">缩放: {{ Number(debugSpriteScale).toFixed(2) }}</label>
              <input v-model.number="debugSpriteScale" type="range" min="0.2" max="2" step="0.1" class="w-full" />
            </div>
            <div>
              <label class="text-xs text-gray-500 block mb-1">透明度: {{ Number(debugSpriteOpacity).toFixed(2) }}</label>
              <input v-model.number="debugSpriteOpacity" type="range" min="0" max="1" step="0.05" class="w-full" />
            </div>
            <div>
              <label class="text-xs text-gray-500 block mb-1">圆角半径: {{ Number(debugBorderRadius).toFixed(2) }}</label>
              <input v-model.number="debugBorderRadius" type="range" min="0" max="0.5" step="0.05" class="w-full"
                :disabled="debugSpriteClip !== 'rounded'" />
            </div>
            <div>
              <label class="text-xs text-gray-500 block mb-1">裁剪模式</label>
              <select v-model="debugSpriteClip"
            ...
            <div>
              ...
              <select v-model="debugSpriteBlendingValue"
                class="w-full bg-white border border-gray-300 text-gray-700 text-xs p-1 rounded">
            ...
            <div>
              ...
              <input v-model="debugSpriteColor" type="color" class="w-full h-8 border border-gray-300 rounded" />
            </div>
            <div class="flex items-center gap-2">
              <input id="sizeAtten" v-model="debugSizeAttenuation" type="checkbox" class="rounded border-gray-300" />
