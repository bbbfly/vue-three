...
export interface PLYLoaderConfig {
  src: string
...
export function usePLYLoader(config: PLYLoaderConfig) {
  const threeCtx = inject(ThreeContextKey)
  const groupCtx = inject(GroupContextKey, null)
  const meshCtx = inject(MeshContextKey, null)

  if (!threeCtx) {
    throw new Error('usePLYLoader must be used within a TCanvas component')
  }
  ...
  const handleMeshParentCase = (geometry: BufferGeometry) => {
    geometry.computeVertexNormals()
    geometry.center()

    if (meshCtx?.setGeometry) {
      meshCtx.setGeometry(geometry)
    }
  }

  const applyShadowToModel = (object: Object3D, config: PLYLoaderConfig) => {
